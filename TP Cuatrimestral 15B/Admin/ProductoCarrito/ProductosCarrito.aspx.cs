...
namespace TP_Cuatrimestral_15B.Admin
{
    public partial class WebForm9 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                RegisterAsyncTask(new PageAsyncTask(CargarProductoCarrito));
            }
        }
        private async Task CargarProductoCarrito()
        {
            var resultado = await gestorProductoCarrito.DevolverProductoCarritos();
            rptProductoCarrito.DataSource = resultado;
            rptProductoCarrito.DataBind();
        }
    }
}
